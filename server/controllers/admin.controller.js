        ...
        for (const row in jsonData) {
            ...
            if (isUser) {
                ...
                for (const cls of teacher.classId) {
                    ...
                    if (!['preprimary-1', 'preprimary-2', 'preprimary-3', 'primary-I-1', 'primary-I-2', 'primary-I-3', 'primary-II-1', 'primary-II-2', 'primary-II-3'].includes(cls)) {
                        err = 1
                        break
                    }
                    else if (isClass) {
                ...
                if (err) {
                    console.log(`Invalid Class or Class already exists \n ${teacher}`)
                    arr.push(0)
                    continue
                } else {
        ...
        for (const row in jsonData) {
            ...
            try {
                if (arr[row]) {
                    ...
                    await teacherModel.create([teacher], { session })
                    for (const cls of teacher.classId) {
                        const l = cls.split('-').length
                        const demoClass = {
                            classId: cls,
                            teacherId: teacher.teacherId,
                            section: l === 3 ? cls.split('-')[0] + '-' + cls.split('-')[1] : cls.split('-')[0],
                            year: l === 3 ? cls.split('-')[2] : cls.split('-')[1],
                            student: []
                        };
                        await classModel.create([demoClass], { session })
                    }
                    uploaded++;
                }
