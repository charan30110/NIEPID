...
function RegisterStudent() {
    ...
    const temporaryFieldsFilled = (obj) => {
        const keys = ["regNo", "dob", "regDate", "name", "sex", "mobileNo", "aadharNo", "section"];
        for (const key in obj.details.info) {
            if (keys.includes(key) && obj.details.info[key].trim() === '') return false
        }
        if (obj.stdCred.section === '') return false
        return true
    }
    ...
    const handleSubmit = async (e) => {
        ...
        //     return;
        // }
        if (!temporaryFieldsFilled(formData)) {
            toast.error("Temporary Fields need to be filled", { position: 'top-right', autoClose: 2500 })
            return;
        }
        ...
        if (aadharNo.length !== 12 || !/^[0-9]{12}$/.test(aadharNo)) {
            toast.error("Aadhar number must be exactly 12 digits and numeric.", { position: 'top-right', autoClose: 2500 });
            return; // Prevent form submission
        }
        if (mno.length !== 10 || !/^[0-9]{10}$/.test(mno)) {
            toast.error("Mobile No. must be 10 digits", { position: 'top-right', autoClose: 2500 });
            return; // Prevent form submission
        }
        ...
        if (registeredDate < bodDate) {
            toast.error("Registration must be greater than Date of Birth.", { position: 'top-right', autoClose: 2500 });
            return; // Prevent further processing
        }
        ...
        if (answer) {
            try {
                await axios.post('http://localhost:4001/admin/registerStudent', formData, {
                    headers: {
                    ...
                    .catch(err => {
                        toast.dismiss()
                        toast.error(err.response.data, { position: 'top-right', autoClose: 2500 });
                        console.log(err)
                        return;
                    })
                // console.log("success");
